...
def example_isochrone_query():
    ...
    result = query_pois_from_isochrone(
        isochrone_gdf=isochrone_gdf, categories=["shopping", "services"], simplify_tolerance=0.001
    )
...
def example_building_custom_query():
    ...
    query = build_poi_discovery_query(geometry=polygon, categories=["healthcare"], timeout=120)

    query_display_limit = 500
    print("Generated Overpass Query:")
    print(query[:query_display_limit] + "..." if len(query) > query_display_limit else query)
