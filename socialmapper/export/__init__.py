...
def export_census_data_to_csv(
    ...
    if "GEOID" in df.columns and not df["GEOID"].empty:
        ...
        try:
            if (
                not df["GEOID"].empty
                and df["GEOID"].iloc[0] is not None
                and len(str(df["GEOID"].iloc[0])) >= 12
            ):
    ...
    if (
        not csv_data.empty
        and "census_block_group" in csv_data.columns
        and "poi_id" in csv_data.columns
    ):
        try:
    ...
    try:
        ...
        fallback_path = "census_data_fallback.csv"
        try:
...
def export_census_data_modern(
    ...
    if config.io.should_use_streaming(estimated_size_mb):
        ...
        return _export_with_streaming(
            census_data, poi_data, output_path, format, include_geometry, config
        )
    else:
        logger.info(f"Using in-memory export for {estimated_size_mb:.1f}MB dataset")
        return _export_in_memory(
            census_data, poi_data, output_path, format, include_geometry, config
        )
