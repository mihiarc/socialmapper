...
def get_geography_hierarchy(
    ...
    client: Optional[TigerGeometryClient] = None,
) -> dict:
    ...
    for level in levels_to_check:
        try:
            ...
            county_fips = hierarchy.get(GeographyLevel.COUNTY, {}).get("GEOID", "")[-3:] if GeographyLevel.COUNTY in hierarchy else None
            ...
            containing = result.geodataframe[
                result.geodataframe.geometry.contains(point)
            ]
...
def get_neighboring_geographies(
    ...
    query = GeometryQuery(
        ...
        state_fips=state_fips,
        county_fips=county_fips if geography_level in [GeographyLevel.BLOCK_GROUP, GeographyLevel.TRACT] else None,
    )
...
def aggregate_to_higher_geography(
    ...
    aggregation: Union[str, dict] = "sum",
) -> gpd.GeoDataFrame:
...
def clip_geometries_to_boundary(
    gdf: gpd.GeoDataFrame,
    boundary: Union[gpd.GeoDataFrame, gpd.GeoSeries],
    keep_all: bool = False,
) -> gpd.GeoDataFrame:
