...
def demo_performance_improvements():
    ...
    try:
        ...
        print(f"   • Pre-computed state relationships: {stats['state_relationships']:,}")
        print(f"   • Pre-computed county relationships: {stats['county_relationships']:,}")
        print(f"   • Cross-state county relationships: {stats['cross_state_county_relationships']:,}")
        print(f"   • Cached point lookups: {stats['cached_points']:,}")
        print(f"   • States with county data: {stats['states_with_county_data']}")
...
def show_migration_example():
    ...
    print("\nOptimized SocialMapper Neighbor API:")
    print("```python")
    print("# Package-level access (simplest)")
    print("import socialmapper")
    print("neighbors = socialmapper.get_neighboring_states('06')  # ~0.3ms")
    print("counties = socialmapper.get_counties_from_pois(pois)   # ~0.2ms per POI")
    print("")
    print("# Dedicated neighbors module (most features)")
    print("import socialmapper.neighbors as neighbors")
    print("nc_neighbors = neighbors.get_neighboring_states_by_abbr('NC')")
    print("stats = neighbors.get_statistics()")
    print("")
    print("# Census module access (original location)")
    print("from socialmapper.census import get_neighboring_states")
    print("neighbors = get_neighboring_states('06')")
    print("```")
    
    print("\nNO SETUP REQUIRED:")
    print("```python")
    print("# Database is packaged with installation - ready to use immediately!")
    print("# 18,560+ county relationships pre-computed")
    print("# 218 state relationships included")
    print("# Sub-millisecond to 2ms lookups")
    print("```")
