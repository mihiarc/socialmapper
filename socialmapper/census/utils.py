...
def migrate_from_old_cache(
    old_cache_dir: Union[str, Path] = "cache",
    db_path: Optional[Union[str, Path]] = None,
    force: bool = False
) -> bool:
    """
    Migrate cached block group data from the old file-based system to DuckDB.
    
    Args:
        old_cache_dir: Path to the old cache directory
    ...
    """
    old_cache_path = Path(old_cache_dir)
    if not old_cache_path.exists():
        get_progress_bar().write(f"Old cache directory not found: {old_cache_path}")
        return False
    
    # Find cached block group files
    cache_files = list(old_cache_path.glob("block_groups_*.geojson"))
    if not cache_files:
        get_progress_bar().write("No cached block group files found to migrate")
        return False
    
    get_progress_bar().write(f"Found {len(cache_files)} cached block group files to migrate")
    ...
    for cache_file in get_progress_bar(cache_files, desc="Migrating cache files", unit="file"):
        try:
            # Extract state FIPS from filename
            filename = cache_file.stem
            if filename.startswith("block_groups_"):
                state_fips = filename.replace("block_groups_", "")
                
                # Check if already exists
                if not force:
                    count = db.conn.execute(
                        "SELECT COUNT(*) FROM block_groups WHERE state_fips = ?", 
                        [state_fips]
                    ).fetchone()[0]
                    
                    if count > 0:
                        get_progress_bar().write(f"State {state_fips} already exists, skipping")
                        continue
                
                # Load and migrate
                gdf = gpd.read_file(cache_file)
                if not gdf.empty:
                    db._store_block_groups(gdf, state_fips)
                    migrated_count += 1
                    get_progress_bar().write(f"Migrated {len(gdf)} block groups for state {state_fips}")
                
        except Exception as e:
            logger.error(f"Failed to migrate {cache_file}: {e}")
            continue
    
    get_progress_bar().write(f"Migration complete: {migrated_count} states migrated")
...
def cleanup_old_cache(
    ...
    if backup:
        backup_path = old_cache_path.parent / f"{old_cache_path.name}_backup"
        if backup_path.exists():
            shutil.rmtree(backup_path)
        shutil.copytree(old_cache_path, backup_path)
        get_progress_bar().write(f"Created backup at {backup_path}")
    ...
    get_progress_bar().write(f"Cleaned up {len(cache_files)} old cache files")
    return True
