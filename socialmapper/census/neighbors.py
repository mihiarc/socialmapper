...
class NeighborManager:
    ...
    def __init__(self, data_path: Optional[Union[str, Path]] = None):
        ...
        if self.system_type == "parquet":
            if ParquetNeighborManager is None:
                raise RuntimeError(
                    "Parquet neighbor system detected but ParquetNeighborManager class not available. Check if neighbors_parquet module can be imported."
                )
        ...
        logger.info(
            f"Initialized NeighborManager with {self.system_type} system at {self.data_path}"
        )
    ...
    def get_neighboring_counties(self, county_fips: str) -> List[str]:
        ...
        neighbor_tuples = self._manager.get_neighboring_counties(
            state_fips, county_code, include_cross_state=True
        )
...
def get_system_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"available": False, "error": str(e), "system_type": NEIGHBOR_SYSTEM}
