...
class NeighborDataManager:
    ...
    def save_state_neighbors(self, df: pd.DataFrame):
        ...
        df = df.astype(
            {"state_fips": "string", "neighbor_state_fips": "string", "relationship_type": "string"}
        )
    ...
    def save_county_neighbors(self, df: pd.DataFrame):
        ...
        logger.info(
            f"Saved {len(df)} county neighbor relationships to {self.county_neighbors_path}"
        )
...
class NeighborManager:
    ...
    def get_neighboring_counties(
        self, state_fips: str, county_fips: str, include_cross_state: bool = True
    ) -> List[Tuple[str, str]]:
        """
        ...
        county_neighbors = df[(df["state_fips"] == state_fips) & (df["county_fips"] == county_fips)]
    ...
    def get_geography_from_point(
        ...
        if use_cache:
            ...
            cached = cache_df[(cache_df["lat"] == lat) & (cache_df["lon"] == lon)]
            ...
            if len(cached) > 0:
                ...
                return {
                    ...
                    "block_group_geoid": (
                        None if pd.isna(row["block_group_geoid"]) else str(row["block_group_geoid"])
                    ),
    ...
    def get_counties_from_pois(
        self, pois: List[Dict], include_neighbors: bool = True, neighbor_distance: int = 1
    ) -> List[Tuple[str, str]]:
        """
    ...
    def get_neighbor_statistics(self) -> Dict[str, Any]:
        ...
        cross_state_count = len(
            county_df[county_df["state_fips"] != county_df["neighbor_state_fips"]]
        )
...
def get_neighboring_counties(
    ...
    """Get neighboring counties for a given county."""
    return get_neighbor_manager().get_neighboring_counties(
        state_fips, county_fips, include_cross_state
    )
...
def get_counties_from_pois(
    pois: List[Dict], include_neighbors: bool = True
) -> List[Tuple[str, str]]:
    """Get counties for POIs with optional neighbors."""
