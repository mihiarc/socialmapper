...
def normalize_variables(variables: list[str]) -> list[str]:
    ...
    ----------
    variables : list of str
        List of variable names (can be codes or readable names).
...
def get_census_data(
    ...
    ----------
    location : Any
        Can be one of: GeoDataFrame with polygon (isochrone), list of
        GEOID strings, tuple of (lat, lon), or dict with 'state_fips'
        and 'county_fips'.
    variables : list of str
        List of census variables (names or codes).
    api_key : str or None, optional
        Census API key, by default None.
    year : int, optional
        Census year, by default 2023.
...
def get_demographics_for_polygon(
    ...
    ----------
    polygon : gpd.GeoDataFrame
        GeoDataFrame with polygon geometry (any source).
    api_key : str or None, optional
        Census API key, by default None.
