...
def sanitize_dataframe_for_json(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for coord_col in ["lat", "lon"]:
        if coord_col in sanitized.columns:
            ...
            try:
                sanitized[coord_col] = pd.to_numeric(sanitized[coord_col], errors="coerce").astype(
                    float
                )
            except:
...
def create_plotly_map(
    ...
    if variable not in census_df.columns:
        available_vars = [
            col for col in census_df.columns if col not in ["geometry", "lat", "lon", "GEOID"]
        ]
    ...
    fig.add_trace(
        go.Scattermap(
            ...
            marker=dict(
                ...
                colorbar=(
                    dict(title=get_variable_label(variable), x=1.02, len=0.8)
                    if show_legend
                    else None
                ),
                opacity=0.7,
            ...
            text=[
                f"<b>Block Group {getattr(row, 'GEOID', 'Unknown')[-6:]}</b><br>"
    ...
    if isochrone_data is not None:
        ...
        if not iso_gdf.empty:
            ...
            for i, (_, iso_row) in enumerate(iso_gdf.iterrows()):
                ...
                if coords:
                    lats, lons = zip(
                        *[(coord[1], coord[0]) for coord in coords[::10]], strict=False
                    ...
                    fig.add_trace(
                        go.Scattermap(
                            ...
                            lon=lons,
                            mode="markers",
                            marker=dict(size=4, color=iso_colors[color_idx], opacity=0.6),
        ...
        if "type" in poi_df.columns:
            for poi_type in poi_df["type"].unique():
                ...
                fig.add_trace(
                    go.Scattermap(
                        ...
                        lon=poi_subset["lon"],
                        mode="markers",
                        marker=dict(size=config["size"], color=config["color"], opacity=0.9),
                        text=[
                            f"<b>{config['emoji']} {getattr(row, 'name', 'POI')}</b><br>"
            ...
            fig.add_trace(
                go.Scattermap(
                    ...
                    lon=poi_df["lon"],
                    mode="markers",
                    marker=dict(size=12, color="#E67E22", opacity=0.9),
                    text=[
                        f"<b>📍 {getattr(row, 'name', 'POI')}</b><br>"
                        f"📍 ({row['lat']:.4f}, {row['lon']:.4f})"
    ...
    fig.update_layout(
        map=dict(style=map_style, center=dict(lat=center_lat, lon=center_lon), zoom=10),
        height=height,
        width=width,
        margin={"r": 0, "t": 50, "l": 0, "b": 0},
        title=dict(
            text=title or f"{get_variable_label(variable)} Distribution", font=dict(size=16), x=0.5
        ),
        showlegend=True,
        legend=dict(
            yanchor="top",
    ...
    if enable_events and PLOTLY_EVENTS_AVAILABLE:
        try:
            ...
            if "JSON serializable" in str(e):
                st.warning(
                    "⚠️ Interactive events disabled due to data serialization issues. Displaying static map."
                )
                ...
                try:
                    ...
                    if "JSON serializable" in str(chart_e):
                        st.error(
                            "❌ Unable to display map due to persistent data serialization issues."
                        )
