...
from socialmapper.visualization.map_utils import (
    apply_quantile_classification,
    calculate_optimal_bins,
    choose_appropriate_colormap,
    get_variable_label,
)
...
def generate_paneled_isochrone_map(
    ...
    for fig_idx in range(num_figures):
        ...
        for i, (iso_path, ax) in enumerate(zip(curr_paths, axes, strict=False)):
            try:
                # Read isochrone data
                ...
                if isochrone.empty:
                    ...
                    ax.text(
                        0.5,
                        0.5,
                        f"No data for {os.path.basename(iso_path)}",
                ...
                for idx, (_, iso) in enumerate(isochrone.iterrows()):
                    ...
                    ax.fill(
                        iso.geometry.exterior.xy[0],
                        iso.geometry.exterior.xy[1],
...
def generate_paneled_census_map(
        ...
        try:
            ...
            if variable_code not in census_df.columns:
                ...
                if variable.lower() in CENSUS_VARIABLE_MAPPING:
                    ...
                    if variable_code not in census_df.columns:
                        print(
                            f"Warning: Variable {variable} ({variable_code}) not found in {census_path}"
                        )
                        processed_census_dfs.append(None)
                        continue
                else:
                    ...
                    if variable.lower() == "poverty_rate":
                        if (
                            "B17001_002E" in census_df.columns
                            and "B17001_001E" in census_df.columns
                        ):
                            census_df[variable] = (
                                census_df["B17001_002E"] / census_df["B17001_001E"] * 100
                            )
                            variable_code = variable
                        else:
                        ...
                        if (
                            "B25003_002E" in census_df.columns
                            and "B25003_001E" in census_df.columns
                        ):
                            census_df[variable] = (
                        ...
                        if (
                            "B25002_003E" in census_df.columns
                            and "B25002_001E" in census_df.columns
                        ):
                            census_df[variable] = (
                                census_df["B25002_003E"] / census_df["B25002_001E"] * 100
                            )
                            variable_code = variable
                        else:
                        ...
                        if (
                            all(var in census_df.columns for var in bachelors_vars)
                            and "B15003_001E" in census_df.columns
                        ):
                            census_df[variable] = (
                                census_df[bachelors_vars].sum(axis=1)
                            ...
                            print(
                                f"Warning: Cannot calculate bachelors_or_higher for {census_path}"
                            )
                            processed_census_dfs.append(None)
                            continue
                    else:
            ...
            if not np.issubdtype(census_df[variable_code].dtype, np.number):
                try:
                    census_df[variable_code] = pd.to_numeric(
                        census_df[variable_code], errors="coerce"
                    )
                except Exception as e:
                    print(
                        f"Warning: Could not convert {variable_code} to numeric in {census_path}: {e}"
                    )
                    processed_census_dfs.append(None)
                    continue
            ...
            if "median" in variable.lower() and "income" in variable.lower():
                ...
                "home_value" in variable.lower()
                or "median" in variable.lower()
                and "value" in variable.lower()
            ):
                # Replace 0 and -666666666 (Census null value) with NaN
                census_df[variable_code] = census_df[variable_code].replace([0, -666666666], np.nan)
    ...
    for fig_idx in range(num_figures):
        ...
        for i, (census_path, census_df, ax) in enumerate(zip(curr_paths, curr_dfs, axes, strict=False)):
            try:
                # Skip if no data
                if census_df is None:
                    ...
                    ax.text(
                        0.5,
                        0.5,
                        f"No data for {os.path.basename(census_path)}",
                        horizontalalignment="center",
                        verticalalignment="center",
                    )
                    continue
