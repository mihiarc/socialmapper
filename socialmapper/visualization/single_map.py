...
def generate_map(
    ...
    if variable not in gdf.columns:
        ...
        if variable_mapped:
            ...
            available_vars = [
                ...
                if col not in ["geometry", "GEOID", "STATE", "COUNTY", "TRACT", "BLKGRP"]
            ]
            raise ValueError(
                f"Variable '{variable}' not found in census data. Available variables: {available_vars}"
            )
    ...
    if gdf[variable].isna().any():
        ...
        if variable in [
            ...
            if len(positive_values) > 0:
                ...
                print("No positive values found, replacing NaN with 0")
    ...
    ctx.add_basemap(ax, source=provider, crs=gdf.crs.to_string(), alpha=0.5)
    ...
    for i in range(len(labels)):
        ...
        legend_handles.append(
            Patch(facecolor=plt.get_cmap(colormap)(norm_value), edgecolor="white", label=labels[i])
        )

    # Add isochrone to legend if it was displayed
    if isochrone_path is not None and show_isochrone and "isochrone" in locals():
        isochrone_legend = Line2D(
            [0], [0], color="blue", linewidth=2, linestyle="-", label=f"{travel_time}-min Isochrone"
        )
    ...
    ax.add_artist(
        ScaleBar(
            1, dimension="si-length", units="m", location="lower right", rotation="horizontal-only"
        )
    )
...
def generate_isochrone_map(
    ...
    ctx.add_basemap(
        ax, source=provider, crs=isochrone.crs.to_string(), alpha=0.7  # Reduce basemap intensity
    )
    ...
    # Add title
    ax.set_title(
        title, fontsize=18, fontweight="bold", fontfamily="Helvetica Neue", pad=20, color="#333333"
    )
    ax.set_axis_off()
