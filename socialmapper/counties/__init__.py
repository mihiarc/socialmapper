...
from socialmapper.states import StateFormat, normalize_state, state_fips_to_name
...
def get_block_groups_for_county(
    state_fips: str, county_fips: str, api_key: Optional[str] = None
) -> gpd.GeoDataFrame:
    """
    ...
    if cache_file.exists():
        try:
            block_groups = gpd.read_file(cache_file, engine="pyogrio", use_arrow=USE_ARROW)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Save to cache
            block_groups.to_file(
                cache_file, driver="GeoJSON", engine="pyogrio", use_arrow=USE_ARROW
            )
...
def get_block_groups_for_counties(
    counties: List[Tuple[str, str]], api_key: Optional[str] = None
) -> gpd.GeoDataFrame:
    """
    ...
    for state_fips, county_fips in get_progress_bar(
        counties, desc="Fetching block groups by county", unit="county"
    ):
        try:
