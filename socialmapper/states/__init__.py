...
def detect_state_format(state: str) -> Optional[StateFormat]:
    ...
    if isinstance(state, str):
        ...
        if (
            ...
            and state.isdigit()
            and state.zfill(2) in STATE_FIPS_TO_ABBR
        ):
...
def normalize_state(
    state: Union[str, int], to_format: StateFormat = StateFormat.ABBREVIATION
) -> Optional[str]:
    """
...
def normalize_state_list(
    states: List[Union[str, int]], to_format: StateFormat = StateFormat.ABBREVIATION
) -> List[str]:
    """
