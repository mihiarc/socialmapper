...
class SpatialGraphBuilder:
        ...
        for i, (node1_id, node1_data) in enumerate(nodes):
            ...
            for dist, neighbor_id in distances:
                if dist <= self.distance_threshold and connected < self.k_neighbors:
                    ...
                    G.add_edge(node1_id, neighbor_id, weight=edge_weight, distance=dist)
                    connected += 1
    ...
    def _check_visibility(self, building1, building2, all_buildings: gpd.GeoDataFrame) -> float:
        ...
        for _, other_building in all_buildings.iterrows():
            if not building1.geometry.equals(
                other_building.geometry
            ) and not building2.geometry.equals(other_building.geometry):
...
if TORCH_GEOMETRIC_AVAILABLE:
    ...
    class GNNCommunityDetector:
        ...
        def __init__(self, model_config: Optional[Dict] = None, use_pretrained: bool = False):
            """
        ...
        def detect_communities(
            self, buildings_gdf: gpd.GeoDataFrame, use_unsupervised: bool = True
        ) -> gpd.GeoDataFrame:
            """
        ...
        def _detect_communities_unsupervised(
            self, buildings_gdf: gpd.GeoDataFrame, G: "nx.Graph"
        ) -> gpd.GeoDataFrame:
            """
        ...
        def _detect_communities_supervised(
            self, buildings_gdf: gpd.GeoDataFrame, G: "nx.Graph"
        ) -> gpd.GeoDataFrame:
            """
...
def detect_communities_with_gnn(
    buildings_gdf: gpd.GeoDataFrame, method: str = "unsupervised", **kwargs
) -> gpd.GeoDataFrame:
    """
