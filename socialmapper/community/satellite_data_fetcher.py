...
class SatelliteDataFetcher:
    ...
    def __init__(
        ...
        logger.info("Cache structure: sentinel2/, naip/, landsat/, metadata/")
    ...
    def _validate_cached_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if file_path.stat().st_size < 1024 * 1024:
                logger.warning(
                    f"Cached file {file_path} too small ({file_path.stat().st_size} bytes)"
                )
                return False
    ...
    def get_best_imagery_for_bounds(
        self,
        bounds: Tuple[float, float, float, float],
    ...
    def _fetch_naip_imagery(
        ...
        try:
            ...
            logger.info(
                f"Searching for NAIP imagery in bounds {bounds} for time range {time_range}"
            )

            # Search for NAIP imagery
            items = geoai.pc_stac_search(
                collection="naip", bbox=bounds, time_range=time_range, max_items=max_items
            )
    ...
    def _fetch_sentinel2_imagery(
        ...
        try:
            ...
            logger.info(
                f"Searching for Sentinel-2 imagery in bounds {bounds} for time range {time_range}"
            )
    ...
    def _fetch_landsat_imagery(
        ...
        try:
            ...
            logger.info(
                f"Searching for Landsat imagery in bounds {bounds} for time range {time_range}"
            )
            ...
            downloaded = geoai.pc_stac_download(
                best_item, output_dir=str(self.landsat_cache), assets=["red", "green", "blue"]
            )
    ...
    def _combine_sentinel2_bands(self, downloaded_info: Dict, output_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if len(band_files) != 3:
                ...
                logger.info(
                    f"Available files: {[f.name for f in recent_files[:10]]}"
                )  # Show first 10
    ...
    def get_imagery_for_buildings(
        ...
        return self.get_best_imagery_for_bounds(bounds=buffered_bounds, imagery_type=imagery_type)
...
def fetch_satellite_imagery_for_community_analysis(
    buildings_gdf: gpd.GeoDataFrame, imagery_type: str = "naip", cache_dir: Optional[str] = None
) -> Optional[str]:
    """
