...
class MultiScaleCommunityAnalyzer:
    ...
    def __init__(self, scales: List[Dict[str, Any]] = None, temporal_window: Optional[int] = None):
        """
    ...
    def analyze_multiscale_communities(
        self,
    ...
    def _hierarchical_analysis(
        self, buildings_gdf: gpd.GeoDataFrame, include_gnn: bool = False
    ) -> Dict[str, gpd.GeoDataFrame]:
        """
    ...
    def _adaptive_analysis(
        ...
        building_density = (
            len(buildings_gdf) / buildings_gdf.total_bounds.area
            if hasattr(buildings_gdf, "total_bounds")
            else 1
        )
    ...
    def _add_scale_metrics(
        self, communities: gpd.GeoDataFrame, scale_config: Dict[str, Any]
    ) -> gpd.GeoDataFrame:
        """
        ...
        for cluster_id in communities["cluster_id"].unique():
            ...
            metrics = {
                ...
                "density_score": len(cluster_buildings)
                / cluster_buildings.geometry.area.sum()
                * 1000000,
            }
    ...
    def analyze_temporal_dynamics(
        self, temporal_data: List[Tuple[datetime, gpd.GeoDataFrame]], track_communities: bool = True
    ) -> pd.DataFrame:
        """
        ...
        for i, (timestamp, buildings_gdf) in enumerate(temporal_data):
            ...
            for scale_name, communities in multiscale_results.items():
                scale_stats = {
                    "timestamp": timestamp,
                    "scale": scale_name,
                    "total_communities": len(communities["cluster_id"].unique())
                    - 1,  # Exclude noise
    ...
    def _calculate_persistence_metrics(
        self, current_communities: gpd.GeoDataFrame, community_tracker: Dict, scale_name: str
    ) -> Dict[str, float]:
        """
    ...
    def export_multiscale_results(
        self, results: Dict[str, gpd.GeoDataFrame], output_dir: str, format: str = "geojson"
    ) -> List[str]:
        """
        ...
        metadata = {
            ...
            "analysis_parameters": {"scales": self.scales, "temporal_window": self.temporal_window},
        }
...
def analyze_communities_multiscale(
    buildings_gdf: gpd.GeoDataFrame,
