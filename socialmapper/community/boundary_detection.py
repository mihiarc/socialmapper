...
class IntegratedCommunityDetector:
    ...
    def discover_community_boundaries(self,
                                    buildings_gdf: gpd.GeoDataFrame,
                                    satellite_image: Optional[str] = None,
                                    demographic_data: Optional[gpd.GeoDataFrame] = None,
                                    existing_boundaries: Optional[gpd.GeoDataFrame] = None) -> gpd.GeoDataFrame:
        """
        ...
        if demographic_data is not None:
            ...
            demographic_features = self._extract_demographic_features(demographic_data, buildings_gdf)
        
        # Step 4: Integrate all features
        print("🔗 Integrating multi-modal features...")
        combined_features = self._combine_features(spatial_features, visual_features, demographic_features)
        ...
        refined_boundaries = self._refine_boundaries(communities, buildings_gdf, existing_boundaries)
    ...
    def _extract_visual_features(self, satellite_image: Optional[str], buildings_gdf: gpd.GeoDataFrame) -> np.ndarray:
        ...
        try:
            ...
            for _, building in buildings_with_visual.iterrows():
                ...
                land_use_features = {
                    'dense_residential': confidence_multiplier if land_use == 'dense_residential' else 0,
    ...
    def _extract_demographic_features(self, demographic_data: gpd.GeoDataFrame, buildings_gdf: gpd.GeoDataFrame) -> np.ndarray:
        """Extract demographic similarity features."""
        # Spatial join to associate demographic data with buildings
        buildings_with_demo = gpd.sjoin(buildings_gdf, demographic_data, how='left', predicate='within')
        
        # Select relevant demographic columns (adjust based on your data)
        demo_columns = [col for col in demographic_data.columns 
    ...
    def _combine_features(self, spatial_features: np.ndarray, 
                         visual_features: Optional[np.ndarray] = None,
                         demographic_features: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if demographic_features is not None:
            combined = np.concatenate([combined, demographic_features * self.demographic_weight], axis=1)
        
        return combined
    
    def _detect_communities(self, features: np.ndarray, buildings_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        final_clusters = self._ensemble_clustering([clusters_hdbscan, clusters_spectral, clusters_network])
    ...
    def _cluster_spectral(self, features: np.ndarray, buildings_gdf: gpd.GeoDataFrame) -> np.ndarray:
    ...
    def _classify_community_type(self, community_buildings: gpd.GeoDataFrame, density: float) -> str:
    ...
    def _validate_against_existing(self, 
                                  detected_boundaries: gpd.GeoDataFrame,
                                  existing_boundaries: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        validated['confidence'] = np.where(np.array(overlaps) > 0.3, 'high', 
                                         np.where(np.array(overlaps) > 0.1, 'medium', 'low'))
...
def discover_community_boundaries(buildings_gdf: gpd.GeoDataFrame,
                                satellite_image: Optional[str] = None,
                                demographic_data: Optional[gpd.GeoDataFrame] = None,
    ...
    from .spatial_clustering import detect_housing_developments
...
def validate_boundaries_with_demographics(boundaries_gdf: gpd.GeoDataFrame,
                                        demographic_data: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
...
def merge_administrative_and_organic_boundaries(administrative_gdf: gpd.GeoDataFrame,
                                              organic_gdf: gpd.GeoDataFrame,
                                              merge_threshold: float = 0.5) -> gpd.GeoDataFrame:
    """
