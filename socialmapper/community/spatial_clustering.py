...
class CommunityBoundaryDetector:
    ...
    def extract_building_features(self, buildings_gdf: gpd.GeoDataFrame) -> np.ndarray:
        ...
        for idx, building in buildings_gdf.iterrows():
            ...
            distances = [
                building.geometry.distance(other.geometry)
            ...
            avg_distance_5nn = (
                np.mean(sorted(distances)[:5]) if len(distances) >= 5 else nearest_distance
            )
    ...
    def create_development_boundaries(
        self, clustered_buildings: gpd.GeoDataFrame
    ) -> gpd.GeoDataFrame:
        """
            ...
            boundaries.append(
                {
                    ...
                    "density": len(cluster_buildings)
                    / boundary.area
                    * 1000000,  # buildings per km²
                }
            )
...
def detect_housing_developments(
    buildings_gdf: gpd.GeoDataFrame, method: str = "hdbscan", **kwargs
...
def cluster_building_patterns(
    buildings_gdf: gpd.GeoDataFrame, feature_weights: Optional[Dict[str, float]] = None
) -> gpd.GeoDataFrame:
    """
...
def identify_natural_boundaries(
    ...
    if boundaries:
        ...
        return gpd.GeoDataFrame(
            columns=["geometry", "boundary_type", "barrier_strength"], crs=area_gdf.crs
        )
