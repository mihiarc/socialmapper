...
def status():
    ...
    table.add_row(
        "Rate Limit", f"{config.rate_limit_requests_per_second} rps", "Requests per second limit"
    )
    table.add_row(
        "Census API Key", "✓" if config.census_api_key else "✗", "Census API key configured"
    )
...
def validate():
    ...
    if (
        config.api_server_host not in ["localhost", "127.0.0.1", "0.0.0.0"]
        and not config.api_server_host.replace(".", "").isdigit()
    ):
