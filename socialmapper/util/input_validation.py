...
def validate_coordinates(lat: Union[str, int, float], lon: Union[str, int, float]) -> tuple[float, float]:
...
def validate_api_response(response_data: Any, expected_fields: Optional[List[str]] = None) -> bool:
...
def validate_numeric_range(value: Union[str, int, float], min_val: float, max_val: float, 
...
def validate_list_input(input_list: Any, min_length: int = 0, max_length: Optional[int] = None,
                       item_type: Optional[type] = None, item_name: str = "item") -> List[Any]:
...
def validate_dict_input(input_dict: Any, required_keys: Optional[List[str]] = None,
