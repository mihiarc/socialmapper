...
class TestExtractGeoidComponents:
    ...
    def test_valid_geoid_extraction(self):
        ...
        df = pd.DataFrame({"GEOID": ["120010001001", "120010001002", "120010002001"]})
...
class TestProcessFipsCodes:
    ...
    def test_state_fips_processing(self):
        ...
        df = pd.DataFrame({"STATE": ["1", "12", "123"]})
    ...
    def test_county_fips_processing(self):
        ...
        df = pd.DataFrame({"STATE": ["12", "6"], "COUNTY": ["1", "59"]})
    ...
    def test_null_values(self):
        ...
        df = pd.DataFrame({"STATE": [None, "12", ""], "COUNTY": ["1", None, "3"]})
...
class TestAddTravelColumns:
    ...
    def test_add_poi_info_from_list(self):
        ...
        poi_data = [{"name": "Test Library", "type": "library", "lat": 37.7749, "lon": -122.4194}]
    ...
    def test_add_poi_info_from_dict(self):
        ...
        df = pd.DataFrame({"id": [1, 2, 3]})
        poi_data = {
            "pois": [{"name": "Test Park", "type": "park", "lat": 40.7128, "lon": -74.0060}]
        }
    ...
    def test_add_travel_time_and_mode(self):
        ...
        result = add_travel_columns(df, poi_data={}, travel_time_minutes=15, travel_mode="walk")
...
class TestReorderColumns:
    ...
    def test_basic_reordering(self):
        ...
        df = pd.DataFrame({"col3": [1, 2, 3], "col1": [4, 5, 6], "col2": [7, 8, 9]})

        config = DataPrepConfig(preferred_column_order=["col1", "col2", "col3"])
    ...
    def test_missing_preferred_columns(self):
        ...
        df = pd.DataFrame({"col2": [1, 2], "col4": [3, 4]})
    ...
    def test_excluded_columns(self):
        ...
        df = pd.DataFrame(
            {"keep1": [1, 2], "exclude1": [3, 4], "keep2": [5, 6], "geometry": [None, None]}
        )

        config = DataPrepConfig(
            preferred_column_order=["keep1", "keep2"], excluded_columns=["exclude1", "geometry"]
        )
...
class TestDeduplicateRecords:
    ...
    def test_custom_aggregation_rules(self):
        ...
        config = DataPrepConfig(
            ...
            deduplication_agg_rules={"min_col": "min", "max_col": "max", "first_col": "first"},
        )
    ...
    def test_no_valid_groupby_columns(self):
        ...
        config = DataPrepConfig(deduplication_columns=["nonexistent_col"])
...
class TestPrepareCensusData:
    ...
    def test_full_preparation_pipeline(self):
        ...
        poi_data = [{"name": "Central Library", "type": "library", "lat": 0.5, "lon": 0.5}]

        result = prepare_census_data(
            census_data, poi_data, travel_time_minutes=15, travel_mode="walk"
        )
    ...
    def test_no_deduplication(self):
        ...
        census_data = gpd.GeoDataFrame({"GEOID": ["BG1", "BG1", "BG2"], "value": [1, 2, 3]})

        result = prepare_census_data(census_data, poi_data={}, deduplicate=False)
    ...
    def test_custom_config(self):
        ...
        census_data = gpd.GeoDataFrame(
            {"GEOID": ["12001"], "custom_col": [100], "exclude_me": [200]}
        )
        ...
        result = prepare_census_data(census_data, poi_data={}, config=custom_config)

        assert "exclude_me" not in result.columns
        assert next(iter(result.columns)) == "custom_col"
