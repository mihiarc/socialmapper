...
def create_figure_1_population_access():
    ...
    for bar, pop in zip(bars1, populations):
    ...
    for i, (inc, label, color) in enumerate(zip(incremental, labels, colors_stack)):
...
def create_figure_2_demographic_analysis():
    ...
    # Add value labels
    for i, (mode, income) in enumerate(zip(df['Mode'], df['Median Income'])):
    ...
    # Add value labels
    for i, (mode, density) in enumerate(zip(df['Mode'], df['Population Density'])):
    ...
    # Add value labels
    for i, (mode, units) in enumerate(zip(df['Mode'], df['Census Units'])):
    ...
    np.random.seed(42)
    income_data = []
    for mode, median in zip(['Walk', 'Bike', 'Drive'], df['Median Income']):
...
def create_figure_3_equity_analysis():
    ...
    # Add value labels
    for bar, ratio in zip(bars1, pop_ratios):
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2., height + 0.05,
                f'{ratio:.1f}x', ha='center', va='bottom', fontsize=9)
    
    for bar, ratio in zip(bars2, income_ratios):
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2., height + 0.05,
