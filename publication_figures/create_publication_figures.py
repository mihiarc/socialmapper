...
def create_figure_1_population_access():
    ...
    incremental = [walk_pop, populations[1] - walk_pop, populations[2] - populations[1]]
    ...
    for _i, (inc, label, color) in enumerate(zip(incremental, labels, colors_stack, strict=False)):
        ax2.bar(
    ...
    ax2.legend(
        loc="center left", bbox_to_anchor=(1, 0.5), frameon=True, fancybox=False, shadow=False
    )
...
def create_figure_2_demographic_analysis():
    ...
    df = pd.DataFrame(
        {
            ...
            "Median Income": [
                data["walk"]["median_income"],
                data["bike"]["median_income"],
                data["drive"]["median_income"],
            ],
            "Population Density": [
                data["walk"]["population_density"],
    ...
    # Add value labels
    for i, (_mode, income) in enumerate(zip(df["Mode"], df["Median Income"], strict=False)):
    ...
    # Add value labels
    for i, (_mode, density) in enumerate(zip(df["Mode"], df["Population Density"], strict=False)):
    ...
    # Add value labels
    for i, (_mode, units) in enumerate(zip(df["Mode"], df["Census Units"], strict=False)):
...
def create_figure_3_equity_analysis():
    ...
    ax1.set_title(
        "A. Population Distribution by\nMinimum Required Travel Mode", fontweight="bold", pad=20
    )
...
def create_summary_table():
    ...
    plt.title(
        "Table 1: Library Accessibility by Travel Mode in Chapel Hill, NC",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
