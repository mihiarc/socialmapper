    ...
    try:
        ...
        if search:
            ...
            filtered_vars = [
                ...
                or search_lower in v.concept.lower()
                or search_lower in v.code.lower()
            ]

        # Get unique categories
        categories = list({v.group for v in CENSUS_VARIABLES if v.group})
        categories.sort()
        ...
        return CensusVariablesResponse(
            variables=paginated_vars, total_count=total_count, categories=categories
        )
    ...
    try:
        ...
        if search:
            ...
            filtered_pois = [
                ...
                if search_lower in p.type.lower()
                or search_lower in p.name.lower()
                or (p.description and search_lower in p.description.lower())
                or (p.common_names and any(search_lower in name.lower() for name in p.common_names))
            ]

        # Get unique categories
        categories = list({p.category for p in POI_TYPES if p.category})
        categories.sort()
        ...
        return POITypesResponse(
            poi_types=paginated_pois, total_count=total_count, categories=categories
        )
...
async def search_locations(
    ...
    try:
        ...
        return LocationSearchResponse(
            query=q, results=limited_results, total_count=len(limited_results)
        )
